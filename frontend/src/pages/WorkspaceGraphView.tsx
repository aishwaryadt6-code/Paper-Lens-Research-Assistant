...
import { aiService } from '../modules/ai-insights/aiService';
...
// Custom Node Component (Visual Grouping & Hover Info)
  ...
  return (
    <div
      className={`group relative px-4 py-3 rounded-xl border text-left transition-all text-xs font-semibold max-w-[200px] shadow-sm ${style.bg} ${style.border} ${
        selected ? 'border-slate-500 ring-1 ring-slate-400' : ''
      }`}
    >
      ...
      <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 hidden group-hover:block z-50 bg-slate-900 text-white text-[10px] p-3 rounded-lg shadow-xl max-w-[240px] pointer-events-none border border-slate-850">
        ...
        {data.authors && <p className="text-slate-355 font-semibold mb-1">Authors: {data.authors}</p>}
...
export default function WorkspaceGraphView() {
  ...
  const { data: papersData, isLoading: papersLoading } = usePapers(workspaceId || '');
  ...
  const fetchGraph = useCallback(async () => {
    if (papersLoading || allPapers.length === 0) return;
    setLoading(true);
    setIsOffline(false);
    try {
  ...
  if (wsLoading || papersLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-[80vh] gap-3">
        <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-brand-500"></div>
        <p className="text-sm text-slate-500 font-semibold">Loading Workspace Graph...</p>
      </div>
    );
  }

  return (
    <div className="space-y-4 pt-3 select-text w-full h-[90vh] flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between pb-2">
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate(`/workspaces/${workspaceId}`)}
            className="flex items-center justify-center h-8 w-8 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 transition-colors"
          >
            <ArrowLeft className="h-4 w-4 text-slate-600 dark:text-slate-300" />
          </button>
          <div>
            <h1 className="text-lg font-bold text-slate-900 dark:text-slate-100">
              {workspace?.name} — Knowledge Graph
            </h1>
            <p className="text-2xs text-slate-400 dark:text-slate-500">
              Visualizing semantic relationship clusters across {allPapers.length} papers
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Controls */}
      <div className="grid sm:grid-cols-4 gap-4 bg-slate-50 dark:bg-slate-900/40 p-4 border border-slate-150 dark:border-slate-850 rounded-2xl shrink-0">
      ...
      {/* Canvas */}
      <div className="flex-1 border border-slate-200 dark:border-slate-850 bg-slate-50 dark:bg-slate-950/40 rounded-2xl overflow-hidden relative">
        {loading ? (
